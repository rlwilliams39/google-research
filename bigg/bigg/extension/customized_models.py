...
class BiggWithEdgeLen(RecurTreeGen):
    ...
    def predict_edge_feats(self, state, edge_feats=None):
        ...
        if edge_feats is None:
            ll = 0
            pred_mean = pred_parameters[0][0]
            pred_lvar = pred_parameters[0][1]
            pred_var = torch.add(torch.nn.functional.softplus(pred_lvar, beta = 1), self.tol)
            edge_feats = torch.FloatTensor([np.random.normal(pred_mean, pred_lvar**0.5)])
        else:
